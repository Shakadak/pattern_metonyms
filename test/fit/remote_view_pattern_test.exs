defmodule Fit.RemoteViewPatternTest do
  ...
  test "view pattern with remote call, and argument" do
    ...
    fit((Map.new(fn x -> {x, x * 2} end) -> %{3 => x}), 1..3)
    result = x + 1
  ...
  test "view pattern with remote call, and argument, and guard" do
    ...
    fit((Map.new(fn x -> {x, x * 2} end) -> %{3 => x}) when x > 5, 1..3)
    result = x + 1
  ...
  test "simple equality pattern" do
    assert_raise MatchError, fn ->
      ...
      fit({(Function.identity() -> x), (Function.identity -> x)}, {1, 2})
      _ = x
    end
  end
